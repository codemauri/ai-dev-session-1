...
import { Category, IngredientCreate, RecipeCreate, api } from '@/lib/api';
...
export default function NewRecipe() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newRecipe = await api.recipes.create(recipeData);

      // If user selected a file to upload, upload it
      if (imageFile) {
        try {
          await api.recipes.uploadImage(newRecipe.id, imageFile);
        } catch (uploadErr) {
          console.error('Image upload failed:', uploadErr);
          // Don't fail the whole operation, just log the error
          // Recipe was created successfully, just without the uploaded image
        }
      }
